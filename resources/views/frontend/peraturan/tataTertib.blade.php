@extends('frontend.default')
@section('title', 'Tata Tertib')

@section('content')
    <div class="containe-fluid py-5 d-flex flex-column align-items-center">
        <h2 class="text-center text-capitalize py-3">::Tata Tertib Perpustakaan::</h2>
    </div>

    <div class="container-fluid d-flex flex-column align-items-center"></div>
    <div class="container" style="padding-bottom: 8rem">
        <div class="row">
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-hand-thumbs-up text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div class="me-1">
                    <h4>1. TAAT PERATURAN</h4>
                    <p>Setiap pengunjung perpustakaan diwajibkan
                        mentaati semua peraturan dan prosedur yang
                        berlaku.</p>
                </div>
            </div>
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="fs-4 bi bi-volume-off-fill text-danger" style="font-size: 8rem;"></i>
                </div>
                <div class="me-1">
                    <h4>2. TENANG & TERTIB</h4>
                    <p>Setiap pengunjung perpustakaan wajib bersama - sama menjaga ketertiban,ketenangan dan keamanan ruang
                        perpustakaan.</p>
                </div>
            </div>
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-recycle text-danger" style="font-size: 1.4rem;"></i>
                </div>
                <div>
                    <h4>3. MENJAGA KEBERSIHAN</h4>
                    <p>Setiap pengunjung diwajibkan menjaga
                        kebersihan demi kenyamanan bersama.</p>
                </div>
            </div>
        </div>
        <div class="row pt-5">
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-trash2-fill text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div class="me-1">
                    <h4>4. BEBAS SAMPAH</h4>
                    <p>Setiap pengunjung diwajibkan membuang
                        sampah di tempat sampah yang telah disediakan.</p>
                </div>
            </div>
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="fs-5 bi bi-person-fill text-danger" style="font-size: 1.4rem;"></i>
                </div>
                <div class="me-1">
                    <h4>5. SOPAN & RAPIH</h4>
                    <p>Setiap pengunjung wajib mengenakan pakaian yang sopan,rapi dan sepatutnya sesuai dengan
                        ketentuan yang berlaku di
                        SMAN 1 Blega.</p>
                </div>
            </div>
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="fs-5 bi bi-list-ul text-danger" style="font-size: 1.4rem;"></i>
                </div>
                <div>
                    <h4>6. HANYA MEMBAWA CATATAN</h4>
                    <p>Setiap pengunjung hanya diperbolehkan
                        membawa catatan atau lembar kertas catatan di
                        ruang baca.</p>
                </div>
            </div>
        </div>
        <div class="row pt-5">
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-handbag-fill text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div class="me-1">
                    <h4>7. TIDAK MEMBAWA TAS</h4>
                    <p>Setiap pengunjung dilarang membawa segala
                        jenis dan bentuk tas atau kantong ke ruang
                        baca.</p>
                </div>
            </div>
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-archive-fill text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div class="me-1">
                    <h4>8. GUNAKAN TEMPAT PENITIPAN</h4>
                    <p>Segala jenis dan bentuk tas atau
                        kantong, jaket dan sejenisnya disimpan di tempat
                        yang telah disediakan. (locker, almari dan rak).</p>
                </div>
            </div>
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-cup-hot-fill text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div>
                    <h4>9. DILARANG MAKAN & MINUM</h4>
                    <p>Setiap pengunjung dilarang membawa
                        makanan dan minuman atau makan dan minum di
                        ruang perpustakaan.</p>
                </div>
            </div>
        </div>
        <div class="row pt-5">
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-slash-circle text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div class="me-1">
                    <h4>10. DILARANG MEROKOK</h4>
                    <p>Setiap pengunjung dilarang merokok di
                        perpustakaan.</p>
                </div>
            </div>
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-save-fill text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div class="me-1">
                    <h4>11. MENJAGA PROPERTI</h4>
                    <p>Setiap pengunjung dilarang melakukan corat - coret, merusak, mengambil sebagian halaman
                        buku, koleksi atau barang milik perpustakaan.</p>
                </div>
            </div>
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-book-half text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div>
                    <h4>12. PENGEMBALIAN BUKU</h4>
                    <p>Setiap pengunjung dilarang mengembalikan
                        sendiri buku yang sudah dibaca atau batal
                        dipinjam ke rak buku. Buku yang batal dipinjam
                        dikembalikan kepada petugas.</p>
                </div>
            </div>
        </div>
        <div class="row pt-5 pb-5">
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-eye text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div class="me-1">
                    <h4>13. PEMERIKSAAN</h4>
                    <p>Petugas berwenang memeriksa tas atau barang
                        yang dibawa pengunjung pada saat masuk
                        atau keluar perpustakaan.</p>
                </div>
            </div>
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-exclamation-triangle-fill text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div class="me-1">
                    <h4>14. SANKSI</h4>
                    <p>Pelanggaran atas Tata Tertib akan dikenakan
                        sanksi sesuai dengan prosedur dan ketentuan
                        yang berlaku di lingkungan
                        SMAN 1 Blega.</p>
                </div>
            </div>
            <div class="col d-flex wow fadeInUp" data-wow-delay="0.3s">
                <div style="padding-right: 15px">
                    <i class="bi bi-pencil-square text-danger" style="font-size: 1.2rem;"></i>
                </div>
                <div>
                    <h4>15. LAIN-LAIN</h4>
                    <p>Hal-hal lain yang belum diatur di dalam Tata
                        Tertib ini akan ditetapkan dan diumumkan
                        kemudian.</p>
                </div>
            </div>
        </div>
    </div>
@endsection
